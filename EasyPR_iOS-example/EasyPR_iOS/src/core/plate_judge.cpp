...
namespace easypr {
  ...
  PlateJudge::PlateJudge() { 
    bool useLBP = false;
    if (useLBP) {
        LOAD_SVM_MODEL(svm_, SGGlobalEasyPRPath::kLBPSvmPath);
      extractFeature = getLBPFeatures;
    }
    else {
        LOAD_SVM_MODEL(svm_, SGGlobalEasyPRPath::kHistSvmPath);
      extractFeature = getHistomPlusColoFeatures;
    }
  }

  void PlateJudge::LoadModel(std::string path) {
      if (path != std::string(SGGlobalEasyPRPath::kDefaultSvmPath)) {
      if (!svm_->empty())
        svm_->clear();
